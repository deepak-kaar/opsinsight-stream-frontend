...
/**
 * `FilterComponent` is responsible for rendering dropdowns for selecting an application and organization,
 * and emitting the selected filter parameters to parent components.
 * 
 * It loads available apps and organizations from the backend using `PageAdministratorService`,
 * maintains the selection using `FilterService`, and emits the current selection via the `filterParam` output.
 */
@Component({
  selector: 'app-filter',
  standalone: false,
...
export class FilterComponent implements OnInit {
  ...
  /**
   * Emits the current filter parameters (application ID and organization ID)
   * to the parent component when filters are applied or changed.
   */
  @Output() filterParam = new EventEmitter<{ appId: string, orgId: string }>();
  ...
  onAppChange(appId: string): void {
    ...
    this.filterParam.emit({ appId: this.selectedApp, orgId: this.selectedOrg });
  }
