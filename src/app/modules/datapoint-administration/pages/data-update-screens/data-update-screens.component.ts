...
@Component({
  selector: 'app-data-update-screens',
  standalone: false,
  templateUrl: './data-update-screens.component.html',
  styleUrl: './data-update-screens.component.css'
})
export class DataUpdateScreensComponent {
  @Input() appId: any;
  items: MenuItem[];
 ...
 /**
  ...
  openEntity(event: any) {
    if (event.targetForm) {
      this.router.navigate(['/globalRenderer/targetEntry', event.idtId]);
    }
    else {
      this.router.navigate(['/globalRenderer/formPage', event.idtId], {
        queryParams: { date: this.date, ch: true }
      });
    }
  }
