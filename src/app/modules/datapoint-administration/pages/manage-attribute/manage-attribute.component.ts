...
export class ManageAttributeComponent implements OnInit {
  ...
  attributeForm: FormGroup = new FormGroup({
    ...
    attrLevel: new FormControl<string>(''),
    ...
    attributeLevelName: new FormControl<string>(''),
    attributeOrgLevel: new FormControl<string>(''),
  })
  products: [] = [];
  appData: any;
  constructor(private router: Router, private activateRoute: ActivatedRoute, private spinner: NgxSpinnerService, private datapointAdminService: DatapointAdministrationService,
    public dialogService: DialogService,
    private messageService: MessageService) {
    this.activateRoute.paramMap.subscribe((params: any) => {
      this.attributeId = params.params.id;
    })
  ...
  getDatas() {
    forkJoin([this.datapointAdminService.getEntityList({}), this.datapointAdminService.getDataPoints(), this.datapointAdminService.getAttributeData({ attributeId: this.attributeId })]).subscribe({
      next: ([res1, res2, res3]) => {
        if (res1) {
          this.entityList = res1.Entity_Attributes.map((item: any) => ({
        ...
        if (res3) {
          this.patchValue(res3.attribute)
        }
      },
      error: (err) => {
        console.log(err);
  ...
  patchValue(attr: any) {
    ...
    if (attr.attrLevel === 'Entity') {
      this.getEntity();
    }
    else if (attr.attrLevel === 'Instance') {
      this.getInstance();
    }
    ...
    this.attributeForm.patchValue({
      ...
      entityId: attr.attrLevel === 'Entity' ? attr?.entityId : attr.attrLevel === 'Instance' ? attr?.instanceId : '',
      nullable: attr.isNull,
      engineeringUnit: attr.engineeringUnit,
      decimalPlaces: attr.decimalPlaces,
      unique: attr.unique,
      timeSeries: attr.timeSeries,
      ...
      alias: attr.alias,
      attributeLevel: attr.attributeLevel,
      attributeLevelName: attr.attributeLevel,
      attributeOrgLevel: attr.attributeLevel,
      attrLevel: attr.attrLevel
    })
    ...
    this.defaultsArray = attr?.defaults || [];
    this.getEntityData(attr.lookupId);
  }
  ...
  submit() {
    if (this.attributeForm.valid) {
      ...
      this.datapointAdminService.updateAttribute(payload).subscribe({
        next: (res: any) => {
          this.spinner.hide();
          this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Attribute Updated Successfully' });
  ...
  closeCallback(e: Event): void {
    this.router.navigateByUrl('/admin/home/attributes');
  }
